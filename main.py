...
from db import get_async_session, UserRepository, CategoryRepository, WorkoutRepository
from chemas import UserCreate, CategoryCreate, WorkoutCreate, UserRead, CategoryRead, WorkoutRead
...
async def create_user_handler(user_data: UserCreate, session: AsyncSession = Depends(get_async_session)):
    user_repo = UserRepository(session=session)
...
async def get_categories_by_user_id(category_id: int, session: AsyncSession = Depends(get_async_session)):
...
async def create_category_handler(category_data: CategoryCreate, session: AsyncSession = Depends(get_async_session)):
    category_repo = CategoryRepository(session=session)
...
async def delete_category_handler(category_id: int, session: AsyncSession = Depends(get_async_session)):
...
async def get_workoutdata_by_id(workoutdata_id: int, session: AsyncSession = Depends(get_async_session)):
...
async def create_workoutdata_handler(workout_data: WorkoutCreate, session: AsyncSession = Depends(get_async_session)):
    workouts_repo = WorkoutRepository(session=session)
...
async def delete_workoutdata_handler(workoutdata_id: int, session: AsyncSession = Depends(get_async_session)):
